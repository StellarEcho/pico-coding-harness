...
- **任务计划（Plan）模块**：新增 `update_plan` 工具，模型可创建并推进最多 6 步的任务计划；计划随每轮 prompt 展示并参与 checkpoint 恢复；模型长时间不建计划时自动生成占位计划。新增 `/plan` 命令。
- **上下文压缩策略层**：按预算压力、步数间隔或手动触发，把旧历史折叠成运行摘要，支持 token 估算与压缩 trace；新增 `/compact` 命令。
- **记忆层可插拔**：定义 MemoryBackend 协议与注册表（默认关键词后端），新增 Skill 注册表骨架和 `/skills` 命令。
- **混合检索评分与技能能力**：记忆召回升级为 tag / 关键词 / 新鲜度混合评分并记录召回原因；技能支持 prompt 片段、工具白名单和 memory hooks；新增 `memory_read` / `memory_update` 工具。
